...
int main(void)
{
    ...
    MGPIO_VoidSetPinMode_TYPE(_GPIOA_PORT,pin0,OUTPUT_SPEED_10MHZ_PUSHPULL);
		...
		CAN_InitTypeDef CAN_InitConfig ={
			...
			ENABLE,//ReceiveFifoLocked  //discard incoming msg
    ...
    while (1)
    {
        ...
        Data[2] = c;
        c++;
        Tx_transmit_polling(0x55A,Data,3,0);

        Rx_Receive_polling(CAN_RX_FIFO0,&RX_Frame,RData);
...
void Tx_transmit_polling(uint16 stdId,uint8* pData,uint8 DLC,uint8 RTR)
{
...
void Rx_Receive_polling(uint8 RX_FIFO,CAN_RX_Frame_t* RX_frame,uint8* pData)
{
